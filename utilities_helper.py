...
def train_and_log(train_dataloader,validation_dataloader,model,device,criterion,optimizer,save_model, run_param,Experiment_param):
    ...
    for epoch in range(run_param['epochs']):
        ...
        if val_loss < best_loss:
            ...
            if save_model:
                # TODO: save in experiment dir
                # model_type
                
                torch.save(model.state_dict(),f'''{directory_path}model_{Experiment_param['model type']}_.pt''')
...
def get_last_modified_file(directory):
    ...
    if not files:  # If the directory is empty, return None
        return None
